...
func (s *Server) Solve2016day4part1(ctx context.Context) (*pb.SolveResponse, error) {
...
func (s *Server) Solve2016day4part2(ctx context.Context) (*pb.SolveResponse, error) {
	data, err := s.loadFile(ctx, "/media/scratch/advent/2017-4.txt")
	if err != nil {
		return nil, err
	}
	...
	ans := ""
	for _, str := range strings.Split(trimmed, "\n") {
		if strings.Contains(trans(str), "object stor") {
			ans = str
		}
	}
