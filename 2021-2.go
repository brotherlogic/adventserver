...
func procSubAdv(str string) int {
	pos := 0
	depth := 0
	aim := 0
...
func (s *Server) Solve2021day2part2(ctx context.Context) (*pb.SolveResponse, error) {
	data, err := s.loadFile(ctx, "/media/scratch/advent/2021-2.txt")
	if err != nil {
		return nil, err
	}
	...
	return &pb.SolveResponse{Answer: int32(procSubAdv(trimmed))}, nil
}
